...
SECURITY_STATUS AcquireClientCredentials(CredHandle *phCreds, TimeStamp *ptsExpiry) {
    ...
    // For client outbound credentials without presenting a specific client certificate:
    SchannelCred.dwCredFormat = 0; // Must be zero if not using SCH_CRED_FORMAT_CERT_CONTEXT/HASH flags
    SchannelCred.cCreds = 0;
    SchannelCred.paCred = NULL;

    // dwFlags:
    // SCH_CRED_AUTO_CRED_VALIDATION - Schannel automatically validates server cert against trusted CAs.
    // SCH_CRED_MANUAL_CRED_VALIDATION - If you want to manually inspect/allow server certs (for leniency).
    // SCH_CRED_NO_DEFAULT_CREDS - Do not use current user's default cert automatically for client auth.
    // SCH_USE_STRONG_CRYPTO - Enforce strong cryptography.
    // SCH_CRED_NO_SERVER_CERT_VALIDATION - EXTREMELY INSECURE: disables all server cert checks. For testing only.

    SchannelCred.dwFlags = SCH_CRED_AUTO_CRED_VALIDATION | // Recommended default for clients
                           SCH_CRED_NO_DEFAULT_CREDS   |
                           SCH_USE_STRONG_CRYPTO;

    // If you wanted the "lenient" server certificate handling from previous discussions:
    // SchannelCred.dwFlags = SCH_CRED_MANUAL_CRED_VALIDATION | SCH_CRED_NO_DEFAULT_CREDS | SCH_USE_STRONG_CRYPTO;
    // Or the highly insecure option for testing:
    // SchannelCred.dwFlags = SCH_CRED_NO_SERVER_CERT_VALIDATION | SCH_CRED_NO_DEFAULT_CREDS;


    SchannelCred.hRootStore = NULL; // Use system default root store for validating server cert.
    SchannelCred.cMappers = 0;
    SchannelCred.aphMappers = NULL;
    SchannelCred.dwSessionLifespan = 0;
    SchannelCred.dwTlsSecretsFlags = 0;

    // Note: grbitEnabledProtocols is NOT a member of SCH_CREDENTIALS.

    SECURITY_STATUS Status = AcquireCredentialsHandle(
        NULL,                 // Principal name (NULL for default)
        UNISP_NAME_W,         // Schannel SSP name
        SECPKG_CRED_OUTBOUND, // Credentials for outbound connection (client)
        NULL,                 // PLUID (NULL for current process logon ID)
        &SchannelCred,        // Schannel-specific data (now SCH_CREDENTIALS)
    ...
    if (Status != SEC_E_OK) {
        fprintf(stderr, "Client: AcquireCredentialsHandle failed with SCH_CREDENTIALS: 0x%lx\n", Status);
        if (Status == SEC_E_SECPKG_NOT_FOUND) {
             fprintf(stderr, "Client: Schannel SSP not found.\n");
        } else if (Status == SEC_E_NO_CREDENTIALS) { // Using the constant that works for you
             fprintf(stderr, "Client: No credentials available.\n");
        }
    } else {
        printf("Client: Credentials acquired successfully using SCH_CREDENTIALS.\n");
    }
    return Status;
}
...
SECURITY_STATUS PerformClientTlsHandshake(SOCKET ServerSocket, CredHandle* phCreds, CtxtHandle* phContext, SEC_CHAR* pszServerName) {
    ...
    while (SecStatus == SEC_I_CONTINUE_NEEDED ||
        ...
        if (SecStatus == SEC_E_OK) {
            ...
            QueryAndPrintProtocol(&hClientContext, FALSE); // FALSE for client
